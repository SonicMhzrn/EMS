...
public class Signup extends JFrame implements ActionListener{
	...
	textField0.setBounds(265, 91, 148, 20);
	contentPane.add(textField0);
	textField0.setColumns(10);
        
        textField = new JTextField();
	textField.setBounds(265, 60, 148, 20);
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            ...
            String checkEmpIdSql = "SELECT * FROM employee WHERE emp_id = ?";
            PreparedStatement checkStmt = con.c.prepareStatement(checkEmpIdSql);
            checkStmt.setString(1, empId);
            ResultSet rs = checkStmt.executeQuery();

            if (rs.next()) {
                // Employee ID exists, proceed with account creation
                String sql = "insert into account(emp_id, username, name, password, sec_q, sec_ans) values(?, ?, ?, ?, ?, ?)";
                PreparedStatement st = con.c.prepareStatement(sql);
                
                st.setString(1, empId);  // Employee ID
                st.setString(2, textField0.getText());  // Username
                st.setString(3, textField_1.getText()); // Name
                st.setString(4, textField_2.getText()); // Password
                st.setString(5, (String) comboBox.getSelectedItem()); // Security Question
                st.setString(6, textField_3.getText()); // Answer
                ...
                JOptionPane.showMessageDialog(null, "Employee ID not found. Please check and try again.");
            }
